jQuery(document).ready(function() {
  $('#quiz_answer_keys-table').dataTable({
    "dom": '<"top"<"row"<"col-md-6 col-sm-8 col-xs-12 new-button form-group"><"col-md-6 col-sm-4 col-xs-12 form-group filter-group" f <"clear-filter quiz_answer_keys-clear-filter">><"col-md-6 col-sm-3 col-xs-12 form-group" l><"col-md-6 col-sm-9 col-xs-12" p>><"row"<"col-sm-12 table-responsive"rt>><"bottom"<"dt-row"<"col-sm-6 deactivate-button"><"col-sm-6 col-xs-12 pull-right" i>>><"clear">',
    "order": [[ 0, "asc" ]],
    "stateSave": true,
    "stateDuration": -1,
    "processing": true,
    "serverSide": true,
    "ajax": $('#quiz_answer_keys-table').data('source'),
    "columnDefs": [ {
      "targets": 'no-sort',
      "orderable": false
    }],
  ...
  $("div.quiz_answer_keys-clear-filter").html("<a href='#' id='quiz_answer_keys-clear-filter'>"+'Clear'+"</a>");
  $('#quiz_answer_keys-clear-filter').on('click', function(event){
    var table = $('#quiz_answer_keys-table').DataTable();
    table
      .state.clear()
      .order( [ 0, "asc" ] )
      .search('')
